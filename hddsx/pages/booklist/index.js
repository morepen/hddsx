...
Page({
    ...
    initData: function () {
      ...
      this.getbooklist(openid, 0);
    },
    toList:function(event){
      var cur_item = event.currentTarget.dataset.id;
      this.setData( {
        curHdIndex:cur_item
    });
      this.getbooklist(this.data.openid,cur_item);
    },
    toUser: function () {
      wx.navigateTo({
  ...
  getbooklist: function (openid,state){
      ...
      wx.request({
        url: app.globalData.url + '/wx/getBookList',
        data: {
          openid: openid,
          state:state
        },
